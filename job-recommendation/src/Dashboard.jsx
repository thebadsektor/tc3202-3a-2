...
const extractTextFromFile = (file) => {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (file.type === 'application/pdf') {
          if (!API_KEY) {
            reject(new Error("Gemini API key is missing. Please configure it in the dashboard."));
            return;
          }
          ...
          try {
            ...
            reject(new Error("Failed to extract text from Word document (.docx). Please ensure the file is valid or try converting to PDF."));
          }
        } else if (file.type === 'application/msword') {
          // Handle legacy .doc files (unsupported)
          reject(new Error("Legacy Word documents (.doc) are not supported. Please convert to .docx or PDF and try again."));
        } else {
        ...
        reject(new Error(error.message || "Failed to parse the file. Please try a different format."));
      }
    };
    ...
    // Use readAsDataURL for PDFs, readAsArrayBuffer for .docx, readAsText for text files
    if (file.type === 'application/pdf') {
      reader.readAsDataURL(file);
    } else if (file.type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
      reader.readAsArrayBuffer(file); // Needed for mammoth.js
    } else if (file.type === 'application/msword') {
      // No reader needed since we reject .doc files
      reader.onerror(new Error("Unsupported .doc format"));
    } else {
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex-grow p-6">
        <div className="bg-white p-6 shadow-sm flex justify-between items-center rounded-lg">
        ...
        {geminiStatus.initialized && (
          <div className={` mt-4 p-6 flex-grow rounded-md ${geminiStatus.enabled ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-yellow-50 text-yellow-700 border border-yellow-200'}`}>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white p-6 rounded-lg border border-blue-100 shadow-sm mb-4">
                

                <div className="mt-6 flex-grow p-6 space-x-4">
